...
EOF
    ...
    # Add iptables rules for UDP redirection
    sudo iptables -t nat -A PREROUTING -p udp --dport 443 -j REDIRECT --to-port $SS_PORT
    sudo iptables -t nat -A OUTPUT -p udp --dport 443 -j REDIRECT --to-port $SS_PORT
    ;;
  4)
    # Install iptables for option 4
    sudo apt install iptables -y
    CONFIG_TEMPLATE=$(cat <<EOF
{
  "inbounds": [
...
# Step 8: Add Sing-box uninstallation instructions to /root/ss2022.txt
UNINSTALL_COMMANDS=$(cat <<EOF

# Debian/Ubuntu 卸载 Sing-box 命令（--purge 会把配置也删除，保留配置请移除该参数）
...
echo "$UNINSTALL_COMMANDS" | sudo tee -a /root/ss2022.txt > /dev/null
